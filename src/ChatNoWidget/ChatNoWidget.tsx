...
function ChatNoWidget({closeChat}: ChatNoWidgetProps) {
  ...
  const getCurrentPage = (): string | null => {
    ...
    if (!currentPage) {
      localStorage.setItem('currentPage', 'user-select');
      return 'user-select';
    }
    return currentPage;
  }
  ...
  const userSelectToMainMenu = (user: string) => {
    setUserType(user);
  ...
  const renderUserTypeSelect = () => (
    <div id="mainMenu">
      {userType && (
        <>
          <button className="menuButton" onClick={() => {userSelectToMainMenu("prospectiveResident")}}>
          ...
          <button className="menuButton" onClick={() => {userSelectToMainMenu("currentResident")}}>
            Current Tenant
          ...
          <button className="menuButton" onClick={() => {userSelectToMainMenu("owner")}}>
            Property Owner
  ...
  return (
    <div id="chatContainer" className={'slideUp'}>
      ...
      {currentPage === 'main-menu' ? renderMainMenu() : currentPage === 'user-select' ? renderUserTypeSelect() : renderChatInterface()}
      {showContactForm && (
